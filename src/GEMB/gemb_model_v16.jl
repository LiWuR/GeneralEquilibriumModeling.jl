...
# Relative to V15:
#
#   - add_net_supply_agent! accepts high-level commodity selectors together
#     with a user-supplied net-supply function and GEM agent keywords;
#   - commodity selectors use the existing CommoditySpace resolution path;
#   - RelativePeriod-aware CommodityRef materialization is reused unchanged;
#   - structured observed-variable references are materialized through the
#     same path as ordinary high-level add_agent!;
#   - the resulting GEM.NetSupplyAgent is inserted through the STEP 2
#     add_agent!(model, agent) path, so registry logic is not duplicated.
#
# V15 STEP 2 adds safe insertion of already constructed GEM agents.
#
# Relative to V14:
#
#   - add_agent!(model, agent::GEM.AbstractNetSupplyAgent) registers an
#     already constructed low-level GEM net-supply agent;
#   - the agent's existing GEM name remains authoritative;
#   - an optional Symbol/AgentRef identity is accepted only when its
#     deterministic flat name matches the existing GEM agent name;
#   - model-specific commodity-index compatibility is checked before
#     registry mutation;
#   - insertion uses the centralized _register_agent! helper from V14.
#
...
# ----------------------------------------------------------------
# Existing low-level GEM-agent insertion
# ----------------------------------------------------------------

function _check_existing_agent_commodity_indices(
    model::GEMBModel,
    agent::GEM.AbstractNetSupplyAgent,
)
    indices =
    ...
    all(
        ...
        "Economic agent $(repr(GEM.agent_name(agent))) contains a commodity " *
        "index outside the GEMBModel commodity range 1:$(n).",
    ))
...
function _existing_agent_identity(
    agent::GEM.AbstractNetSupplyAgent,
    ::Nothing,
)
    flat_name =
...
function _existing_agent_identity(
    ...
    flat_name == existing_flat_name || throw(ArgumentError(
        "The supplied GEMB agent identity $(repr(agent_ref)) resolves to flat " *
        "GEM name $(repr(flat_name)), but the existing low-level agent is " *
        "named $(repr(existing_flat_name)). Existing agents are registered " *
        "without renaming; construct the GEM agent with the matching flat " *
        "name first.",
    ))
...
Register an already constructed low-level GEM net-supply agent in `model`.

If `name` is omitted, the agent's existing GEM `Symbol` name is retained and
the corresponding high-level identity is `AgentRef(GEM.agent_name(agent))`.

A `Symbol` or absolute `AgentRef` may be supplied through `name` when a
specific GEMB identity should be retained. Its deterministic flat GEM name
must exactly equal `GEM.agent_name(agent)` because this method registers an
existing low-level agent and does not rename or rebuild it.

Before any model registry is mutated, the method verifies that every commodity
index used by the agent lies inside the current `GEMBModel` commodity range.
Identity duplicates and flat-name collisions are checked by the common
registration path.

This method performs no commodity-name resolution and no agent construction.
"""
function add_agent!(
    model::GEMBModel,
    agent::GEM.AbstractNetSupplyAgent;
    ...
    _check_existing_agent_commodity_indices(
        model,
        agent,
    )
...
# ----------------------------------------------------------------
# High-level custom net-supply-agent construction
# ----------------------------------------------------------------
...
Construct and add a custom `GEM.NetSupplyAgent` using high-level GEMB
commodity selectors.

`commodities` accepts the same high-level selector forms used elsewhere in
`GEMBModel`: a `Symbol`, a `CommodityRef`, or a vector containing either.
Selectors are materialized in the context of `name` before they are resolved
to the integer commodity indices required by GEM.

All remaining keyword arguments are forwarded to `GEM.NetSupplyAgent`.
Typical arguments include `variable_names`, `variable_lower_bounds`,
`variable_upper_bounds`, `variable_start`, `observed_variables`, and
`condition_rule`.

High-level structured observed-variable references are materialized in the
same way as in specification-based `add_agent!`.

This is an advanced escape hatch for economic agents whose complete net-supply
mapping and condition rule are supplied directly by the user. Standard GEMB
behavioral specifications should remain the preferred interface when they
naturally represent the economic behavior.
"""
function add_net_supply_agent!(
    model::GEMBModel,
    net_supply_function;
    commodities,
    name,
    ...
    # Reject identity conflicts before doing any custom-agent construction.
    _check_new_agent_identity(
        model,
        agent_ref,
        flat_name,
    ...
    isempty(commodity_indices) && throw(ArgumentError(
        "add_net_supply_agent! requires commodities to resolve to at least " *
        "one concrete commodity.",
    ))
