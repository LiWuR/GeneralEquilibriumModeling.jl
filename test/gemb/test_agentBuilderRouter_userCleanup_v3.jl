...
function _router_user_files(root::String)
    ...
    for directory in (
        joinpath(root, "examples", "gemb",  "gemb"),
        joinpath(root, "docs", "src", "gemb",  "gemb"),
    )
        isdir(directory) ||
            continue
...
@testset "Agent builder router user-facing cleanup" begin
    ...
    new_example =
        joinpath(root, "examples", "gemb", "gemb_CES_CET_nc4_na2_v2.jl",
        )

    old_example =
        joinpath(root, "examples", "gemb", "gemb_CES_CET_nc4_na2_v1.jl",
        )

    new_doc_example =
        joinpath(root, "docs", "src", "gemb", "examples",
