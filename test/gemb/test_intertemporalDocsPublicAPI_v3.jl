...
@testset "Intertemporal docs public API V3" begin
    ...
    docs_root =
        joinpath(package_root, "docs", "src", "gemb", )
    ...
    roots =
        (
            joinpath(package_root, "examples", "gemb", ),
            docs_root,
        )
    ...
    @test occursin(
        "AgentTemplate",
        read(
            joinpath(package_root, "examples", "gemb", "gemb_intertemporalEquilibriumCESClaim_nct3_nat3_v3.jl",
            ),
            String,
        ),
    )
    ...
    @test occursin(
        "add_agent!",
        read(
            joinpath(package_root, "examples", "gemb", "gemb_intertemporalEquilibriumCES_nct2_nat2_v5.jl",
            ),
            String,
        ),
    )
